...
export default class extends Page {
  constructor ({ camera, renderer, size }) {
    ...
    this.exhibitsLength = this.exhibits.length - 1

    each(Exhibits, (exhibits, index) => {
      const media = new Media({
        brand: exhibits.brand,
        color: exhibits.color,
        index,
    ...
    this.widthMinimum = 0
    this.widthMaximum = -this.width * this.exhibitsLength
  ...
  getSpacing (camera) {
    ...
    const projection = new Vector3(0, childPosition.y + child.height / 2, 0).exhibit(camera)
  ...
  onSelect (exhibit) {
    this.emit('change', `/exhibit/${exhibit}`)
  }
